...
class MapVM(var view: IActivity, var model: IMapModel) : IMapVM {
    ...
    override fun onResume() {
        model.stopButton.set(ButtonHandler(Consumer{ geo() }, R.string.map_goPosition, view))
        model.stopButton1.set(ButtonHandler(Consumer{ toMap() }, R.string.map_geo, view))
        model.nextButton.set(ButtonHandler(Consumer{ fakeStart() }, R.string.map_fakeStart, view))
        model.startButton.set(ButtonHandler(Consumer{ home() }, R.string.map_home, view))
        model.startButton1.set(ButtonHandler(Consumer{ hide() }, R.string.hide_panel_open, view))
        model.nextButton1.set(ButtonHandler(Consumer{ mapType() }, R.string.map_mapType, view))
