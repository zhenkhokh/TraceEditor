...
public enum TRACE_PLOT_STATE  {END_COMMAND
	,START_COMMAND
	,CONNECT_COMMAND
	,CENTER_COMMAND
	,CENTER_END_COMMAND
	,CENTER_START_COMMAND
	,CENTER_CONNECT_COMMAND
	,DONOTHING_COMMAND};
	...
	public void setCommand(TRACE_PLOT_STATE command){
		switch(command){
			case START_COMMAND:
				{startPass=true;endPass=false;centerPass=false;break;}
			case CONNECT_COMMAND:
				{startPass=true;endPass=true;centerPass=false;break;}
			case CENTER_START_COMMAND:
				{startPass=true;endPass=false;centerPass=true;break;}
			case CENTER_CONNECT_COMMAND:
				{startPass=true;endPass=true;centerPass=true;break;}
			case END_COMMAND:
				{startPass=false;endPass=true;centerPass=false;break;}
			case CENTER_END_COMMAND:
				{startPass=false;endPass=true;centerPass=true;break;}
			case CENTER_COMMAND:
				{startPass=false;endPass=false;centerPass=true;break;}
			case DONOTHING_COMMAND:
				{startPass=false;endPass=false;centerPass=false;break;}
		}
	}
		...
		for (int i = 0; i < 3; i++) {
			...
			if (ok){
				...
				if (i==2){
					...
					try{titleMarker = getQueryParameter(uri, "title");					
					}catch(Exception e){
    ...
    static public Intent createIntentForExistingCenterEndStart(TRACE_PLOT_STATE states, String pointCenter, Intent in) throws Exception{
    	...
    	switch(states){
    		case START_COMMAND: {util.setStart(center);break;}
    		case END_COMMAND:{util.setEnd(center);break;}
    		...
    		case CENTER_START_COMMAND:{util.setStart(center);util.setCenter(center);break;}
    		case CENTER_END_COMMAND:{util.setEnd(center);util.setCenter(center);break;}
    		case CENTER_COMMAND:{util.setCenter(center);break;}
    	}
    ...
    static public LatLng getGeoPosition(Context ctx){
        LocationManager lManager =  (LocationManager) ctx.getSystemService(Context.LOCATION_SERVICE);
        Location location = getLastBestLocation(lManager);
        LatLng center = null;
        if (location!=null)
    ...
    static private Location getLastBestLocation(LocationManager mLocationManager) {
